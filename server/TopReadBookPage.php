...
$query = "SELECT b.Book_Name, COUNT(DISTINCT s.User_ID) AS number_of_readers
        FROM books b
        LEFT JOIN statistics s ON b.Book_ID = s.Book_ID
        WHERE s.Datetime_Start BETWEEN '$startMonth-01' AND LAST_DAY('$endMonth-01')
        GROUP BY b.Book_ID, b.Book_Name
        ORDER BY number_of_readers DESC, b.Book_Name ASC
        LIMIT 10";
...
<head>
    ...
    <title>รายชื่อหนังสือที่มีจำนวนผู้อ่านมากที่สุด 10 อันดับแรก</title>
    <link rel="preconnect" href="https://fonts.googleapis.com" />
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin />
    <link href="https://fonts.googleapis.com/css2?family=Anuphan&display=swap" rel="stylesheet" />
    <link rel="stylesheet" href="style/styleTopReadBookPage.css" />
</head>

<body>
    <?php include('nav.php'); ?>
    <header class="header">
        <p>รายชื่อหนังสือที่มีจำนวนผู้อ่านมากที่สุด 10 อันดับแรก</p>
    </header>
    ...
    <form method="POST">
        <div class="month-years">
            ...
            <button name="submit" type="submit">แสดงผล</button>
        </div>
    </form>
            ...
            <?php if (count($topBooks) > 0): ?>
                <?php foreach ($topBooks as $index => $book): ?>
                    <div class="top10-item">
                        ...
                        <span><?php echo htmlspecialchars($book['number_of_readers']); ?></span>
                    </div>
                <?php endforeach; ?>
            <?php else: ?>
                <div class="top10-item">
