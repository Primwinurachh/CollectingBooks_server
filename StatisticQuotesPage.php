...
$endMonth = isset($_POST['month-end']) && !empty($_POST['month-end']) ? $_POST['month-end'] : date('Y-m');
    ...
    $query = "SELECT DATE_FORMAT(Datetime_Add_Quote, '%Y-%m') AS month,
        COUNT(Quote_ID) AS total_quotes,
        SUM(Number_of_Like) AS total_likes
            FROM quotes
            WHERE Datetime_Add_Quote BETWEEN '$startMonth-01' AND LAST_DAY('$endMonth-01')
            GROUP BY month
            ORDER BY month";
...
<head>
    ...
    <link rel="stylesheet" href="style/styleStatisticQuotesPage.css" />
    ...
    <?php if ($data): ?>
        ...
        <script>
            ...
            chartData.forEach(function(item) {
                ...
                quotesData.push(item.total_quotes);
                likesData.push(item.total_likes);
            });
            ...
            var myChart = new Chart(ctx, {
                ...
                data: {
                    ...
                    datasets: [{
                        ...
                        label: 'จำนวนการกดไลก์ในแต่ละเดือน',
                        data: likesData,
